...
let getViewForSwagger2 = function (swagger) {
  ...
  _.forEach(swagger.paths, function (api, path) {
    ...
    _.forEach(api, function (op, m) {
      ...
      let method = {
        path: path,
        methodName: op.operationId ? normalizeName(op.operationId) : getPathToMethodName(m, path),
...
let getViewForSwagger1 = function (swagger) {
  let data = {
...
let getViewForOpenApi = function (swagger) {
  let authorizedMethods = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'COPY', 'HEAD', 'OPTIONS', 'LINK', 'UNLIK', 'PURGE', 'LOCK', 'UNLOCK', 'PROPFIND']
  let data = {
    description: swagger.info.description,
    isSecure: swagger.securityDefinitions !== undefined,
...
let parseApi = function (swagger) {
  let data = ''
  let enumerationData = ''
  if (swagger.swagger) { // swagger1.0 / 2.0
    data = swagger.swagger === '1.0' ? getViewForSwagger1(swagger) : getViewForSwagger2(swagger)
  } else {
    data = getViewForOpenApi(swagger)
